...
public class FXResourceBundle {
    ...
    public Map<String, String> getStrings(Set<String> keys) {
        return keys.stream().collect(Collectors.toMap(k -> k, k -> getString​(k, (Object[])null)));
    }
	...
	public StringBinding getStringBinding(String key, Object... args) {
		return Bindings.createStringBinding(() -> getString​(key, args), LocaleProperty());
	}

	public StringBinding getStringBinding(ReadOnlyObjectProperty<?> key, Object... args) {
		return Bindings.createStringBinding(() -> getString​(key.getValue().toString().toLowerCase(), args), key,
				LocaleProperty());
	}
